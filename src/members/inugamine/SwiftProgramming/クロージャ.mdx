...
クロージャは、[変数と定数](/@inugamine/SwiftProgramming/変数と定数)への参照を、それらを定義したコンテキストから *キャプチャ* して保持できる。これは、これらの定数と変数をスコープに*閉じ込める*とも呼ばれる。  
Swift は、キャプチャに関連したすべてのメモリ管理を行う。

[関数](/@inugamine/SwiftProgramming/関数)で取り扱ったグローバル関数やネスト関数は、実際にはクロージャの特殊なケースである。
...
**ネスト関数**は便利だが、毎回名前をつけ完全な形で書く必要があるため、たびたび面倒な時がある。  
そこで、**インラインで短く書くことができ、名前が不要な関数**のようなものを使う。それが**クロージャ式**である。
...
なぜ定数なのに呼ぶ度に結果が変わるのか？  
これは「`counterA` が**どのクロージャを指す**かは変わらない」という意味である。クロージャの**中身** (キャプチャした `count`) が変わるのは問題ない。
